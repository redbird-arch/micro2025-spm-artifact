...
#pragma omp parallel for shared(conn, n1, n2, l1) private(k, j) reduction(+: sum) schedule(static)
  ...
  for (j = 1; j <= n2; j++) {
    ...
    printf("tid = %d, n1 = %d, n2 = %d\n", tid, n1, n2);

    /*** Compute weighted sum of its inputs ***/
    sum = 0.0;
    for (k = 0; k <= n1; k++) {
      // l1[k] is the shared data
      #ifdef EN_PREFETCH
      _mm_prefetch(l1 + k + 16, _MM_HINT_T0);
      #endif
      sum += conn[k][j] * l1[k]; // mulss access l1[k]
    }
    l2[j] = squash(sum);
  }
}
...
#pragma omp parallel for shared(oldw, w, delta) private(j, k, new_dw)          \
  ...
  for (j = 1; j <= ndelta; j++) {
    int tid = omp_get_thread_num();
    printf("tid = %d, ndelta = %d, nly = %d\n", tid, ndelta, nly);
    for (k = 0; k <= nly; k++) {
      // ly[k] is the shared data
      #ifdef EN_PREFETCH
      _mm_prefetch(ly + k + 16, _MM_HINT_T0);
      #endif
