...
long transform_to_cuda(node *root, bool verbose) {
  ...
  while (queue != NULL) {
    ...
    k->keys[0] = INT_MIN;
    for (i = k->num_keys - 1; i < order; i++) {
      // Clear higher keys to INT_MAX;
      k->keys[i] = INT_MAX;
    }
    if (!k->is_leaf) {
      /**
       * For internal nodes:
       * The original tree has (num_keys + 1) pointers.
       *   k1  k2  k3
       * p1  p2  p3  p4
      ...
      /**
       * For leaf nodes:
       * The original tree has num_keys pointers.
       * k1 k2 k3
       * p1 p2 p3
       * Transfor into:
       * -inf k1   k2   k3   +inf
      ...
      for (int i = 0; i < n->num_keys; ++i) {
        k->keys[i + 1] = n->keys[i];
        krecords[recordindex].value = ((record *)n->pointers[i])->value;
        k->indices[i + 1] = recordindex++;
      }
    }

    // My last indice should be next sibling.
  ...
  for (int i = 0; i < queueindex; ++i) {
    knode *k = &knodes[i];
    if (!k->is_leaf) {
      for (int j = 0; j < k->num_keys; j++) {
        assert(k->indices[j] < queueindex);
      }
    } else {
      for (int j = 0; j < k->num_keys; j++) {
        assert(k->indices[j] < size);
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  while (sscanf(commandPointer, "%c", &instruction) != EOF) {
    ...
    case 'k': {
      ...
      kernel_query(cores_arg, records, knodes, knodes_elem, order, maxheight,
                   count, keys, ans);
    ...
    case 'j': {
      ...
      kernel_range(cores_arg, knodes, knodes_elem, order, maxheight, count,
